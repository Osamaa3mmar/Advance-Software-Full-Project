...
class MedicalNeedsRepository {
  ...
  static async updateMedicalNeedStatus(id, newStatus, user) {
    ...
    if (user.role === "PATIENT") {
      ...
      throw new Error("Only patients and organizations can update status.");
    }
    ...
    if (result.affectedRows === 0) {
      throw new Error("Medical need not found or not authorized to update.");
    }
  ...
  static async updateMedicalNeed(id, needData, user) {
    ...
    const [result] = await connection.query(
      `UPDATE medical_needs SET name = ?, description = ?, type = ?, price = ?, quantity = ? WHERE id = ? AND ${condition}`,
      params
    );
  ...
  static async deleteMedicalNeed(id, user) {
    ...
    const [result] = await connection.query(
      `DELETE FROM medical_needs WHERE id = ? AND ${condition}`,
      params
    );

    if (result.affectedRows === 0) {
      throw new Error("Medical need not found or not authorized to delete.");
    }
