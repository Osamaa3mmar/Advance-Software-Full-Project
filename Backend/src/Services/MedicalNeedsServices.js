...
class MedicalNeedsService {
  ...
  static async createMedicalNeed(needData, user) {
    ...
    // Verified patients can create medical needs
      ...
      const isVerified = await MedicalNeedsRepository.checkPatientVerification(
        user.id
      );
      if (!isVerified) {
        throw new Error("Only verified patients can create medical needs");
      }
  ...
  static async updateMedicalNeedStatus(id, newStatus, user) {
    ...
    if (!["PENDING", "CANCELED"].includes(newStatus)) {
      throw new Error("Invalid status. Only 'PENDING' or 'CANCELED' allowed.");
    }
  ...
  static async updateMedicalNeed(id, needData, user) {
    if (!user) {
      throw new Error("Authentication required");
    }

    if (user.role !== "ORGANIZATION" && user.role !== "PATIENT") {
      throw new Error(
        "Only organizations and patients can update medical needs"
      );
    }

    await MedicalNeedsRepository.updateMedicalNeed(id, needData, user);
    return await MedicalNeedsRepository.getMedicalNeedById(id);
  }

  static async deleteMedicalNeed(id, user) {
    if (!user) {
      throw new Error("Authentication required");
    }
    ...
    if (user.role !== "ORGANIZATION" && user.role !== "PATIENT") {
      throw new Error(
        "Only organizations and patients can delete medical needs"
      );
    }

    return await MedicalNeedsRepository.deleteMedicalNeed(id, user);
  }
}
